...
class HOdata(ABC):
    ...
    def __init__(self, **cfg):
        ...
        dex_gt_file = open('common/cache/DexYCB/train.txt', 'r').readlines()
        ho3d_gt_file = open('common/cache/HO3D/train.txt', 'r').readlines()
        self.gt_idx = [s.strip() for s in ho3d_gt_file]
        self.gt_idx.extend([s.strip() for s in dex_gt_file])
    ...
    def get_hand_verts_vis(self, idx):
        if self.data_split not in ["train", "trainval"]:
            verts_vis = np.ones(778)
        else:
            verts_2d = self.get_hand_verts_2d(idx)
            verts_vis = ((verts_2d[:, 0] >= 0) &
                          (verts_2d[:, 0] < self.raw_size[0])) & ((verts_2d[:, 1] >= 0) &
                                                                   (verts_2d[:, 1] < self.raw_size[1]))
    ...
    def __getitem__(self, idx):
        ...
        if self.aug:
            ...
            if self.max_rot > 0:
                ...
                if np.random.rand(1)[0] < 0.5: # at a half time, disable random
                    rot_rad = 0
            else:
                rot_rad = 0 # allow disable rot
        else:
            rot_rad = 0
        ...
        # cam_intr = affine_transf.dot(cam_intr)
        sample[Queries.CAM_INTR] = cam_intr.astype(np.float32)

        joints_3d = rot_mat.dot(joints_3d.transpose(1, 0)).transpose()
        root_joint = joints_3d[self.center_idx] # center_idx is 9, not 0, this is relative to the root at the bottom of middle finger
        ...
        verts_2d = transform.transform_coords(verts_2d, affine_transf).astype(np.float32)
        ...
        if self.data_split not in ["train", "trainval"]:
            ...
            sample[Queries.HAND_VERTS_VIS] = np.full(778, 1.0, dtype=np.float32)
            ...
            sample[Queries.HAND_VERTS_VIS] = np.full(778, 0.0, dtype=np.float32)
        else:
            joints_vis_aug = (((joints_2d[:, 0] >= 0) & (joints_2d[:, 0] < self.image_size[0])) &
                              ((joints_2d[:, 1] >= 0) & (joints_2d[:, 1] < self.image_size[1]))).astype(np.float32)
            hand_verts_vis_aug = (((verts_2d[:, 0] >= 0) & (verts_2d[:, 0] < self.image_size[0])) &
                              ((verts_2d[:, 1] >= 0) & (verts_2d[:, 1] < self.image_size[1]))).astype(np.float32)
            if joints_vis_aug.sum() < CONST.NUM_JOINTS * 0.4:  # magic number
                sample[Queries.JOINTS_VIS] = np.full(CONST.NUM_JOINTS, 0.0, dtype=np.float32)
                sample[Queries.HAND_VERTS_VIS] = np.full(778, 0.0, dtype=np.float32)
            else:
                sample[Queries.JOINTS_VIS] = joints_vis_aug
                sample[Queries.HAND_VERTS_VIS] = hand_verts_vis_aug
        ...
        sample[Queries.OBJ_NAME] = self.get_obj_name(idx)
        sample[Queries.IMAGE_PATH] = self.get_image_path(idx)
        ...
        sample[Queries.WRIST_JOINT] = self.get_joints_3d(idx).reshape(21, 3)[0] # wrist joint, artiboost uses palm joint
        sample[Queries.OBJ_ONE_K] = self.get_obj_onek(idx)
        sample[Queries.OBJ_VERTS_CAN] = self.get_obj_verts_can(idx)[0]

        hand_verts_raw = self.get_hand_verts_3d(idx)
        hand_verts_raw = rot_mat.dot(hand_verts_raw.transpose(1, 0)).transpose()
        sample[Queries.HAND_VERTS_3D] = hand_verts_raw

        obj_verts_raw = self.get_obj_verts_transf(idx)
        obj_verts_raw = rot_mat.dot(obj_verts_raw.transpose(1, 0)).transpose()
        sample[Queries.OBJ_VERTS_3D] = obj_verts_raw

        sample[Queries.OBJ_FACES] = self.get_obj_faces(idx)
        sample[Queries.HAND_FACES] = self.get_hand_faces(idx)
        sample[Queries.IS_VALID] = torch.tensor(self.get_image_path(idx) in self.gt_idx and rot_rad == 0)
